...
void dmp_delay_ms_rt_thread(rt_int32_t ms)//移植
{
...
#define delay_ms    dmp_delay_ms_rt_thread
...
//  IIC_Init();                 //初始化IIC总线
    if (mpu_init(&int_param) == 0) //初始化MPU9250
    {
        ...
        if (res)return 1; //rt_kprintf("mpu_dmp_init is ERR(1) ! 0x%02x\r\n",res);
        res = mpu_set_sensors(INV_XYZ_GYRO | INV_XYZ_ACCEL | INV_XYZ_COMPASS); //设置所需要的传感器
        if (res)return 2; //rt_kprintf("mpu_dmp_init is ERR(2) ! 0x%02x\r\n",res);
        res = mpu_configure_fifo(INV_XYZ_GYRO | INV_XYZ_ACCEL); //设置FIFO
        if (res)return 3; // rt_kprintf("mpu_dmp_init is ERR(3) ! 0x%02x\r\n",res);
        res = mpu_set_sample_rate(DEFAULT_MPU_HZ);              //设置采样率
        if (res)return 4; //rt_kprintf("mpu_dmp_init is ERR(4) ! 0x%02x\r\n",res);
        res = mpu_set_compass_sample_rate(1000 / COMPASS_READ_MS); //设置磁力计采样率
        if (res)return 5; //rt_kprintf("mpu_dmp_init is ERR(5) ! 0x%02x\r\n",res);
...
//返回值:0,正常
//    其他,失败
rt_uint8_t mpu_dmp_get_data(float *pitch, float *roll, float *yaw)
{
